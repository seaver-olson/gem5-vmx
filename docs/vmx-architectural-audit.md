...
EPT, VPID, APIC virtualization, or MSR load/store lists.  VM-entry event
injection is implemented for the vectored event classes described in the
feature matrix below.  Within that target, VM entry changes the page-table root used by instruction and data
...
| VM-entry validation | Implemented; integration verification pending | Capability masks, counts, host/guest control state, selectors, bases, limits, access rights, RFLAGS, EFER, canonicality, activity, interruptibility-state bit-level rules, link pointer, CR3, paging relationships, and guest `IA32_SYSENTER_ESP/EIP` are checked. A focused transition case covers the new failure path, but cannot run without the full-system resources. Unsupported VM-exit MSR list counts fail with error 7; the event-injection descriptor (type, vector, deliver-error-code bidirectionally tied to the specific vectors that architecturally push one, error-code reserved bits, and instruction length, including the MOV-SS-blocking interaction) is validated per SDM 26.2.1.3; a zero instruction length for a software-class injection is rejected unless `IA32_VMX_MISC[30]` is set (it is always clear in this model); a nonzero VM-entry MSR-load count now fails late with exit reason 34 instead of error 7. |
...
| INVD / GETSEC / XSETBV | Partial interception only | Higher-priority recognition and privilege faults precede each distinct non-root VM exit: INVD checks CPL; XSETBV checks CPUID.XSAVE and CR4.OSXSAVE before CPL; and GETSEC checks CR4.SMXE. Root execution after those checks retains baseline gem5's warning/no-op implementation and is outside this VMX research contract. |
...
| MSR load/store lists | Recognized and rejected | VM-exit MSR-store/load counts must still be zero at entry (pre-entry error 7). A nonzero VM-entry MSR-load count is now classified correctly as a late failure (exit reason 34, SDM 26.4) rather than a pre-entry error, but list execution itself remains absent, so entry still cannot succeed with a nonzero count. |
| VM-entry event injection | Implemented for the vectored event classes | External interrupt, NMI, hardware exception (with or without an error code), and the three software-class events (software interrupt, privileged software exception, software exception) are validated per SDM 26.2.1.3 and delivered through the existing microcode gate-dispatch routine after guest-state loading, before the caller resumes at guest RIP. Injecting while the guest-interruptibility-state field indicates MOV-SS blocking fails VM entry with error 26. Blocking-by-NMI is tracked across entry/exit as a direct effect of NMI injection. The STI-blocking/MOV-SS-blocking bits loaded from the interruptibility-state field are honored for one guest instruction after entry (MOV-SS-blocking also withholds NMI, per SDM 6.8.3) by tying the shadow to the entry RIP rather than a general instruction-boundary hook, since gem5 has none; this is scoped to VMX non-root execution only and does not model `STI`/`MOV SS` shadowing for native execution or for the VMLAUNCH/VMRESUME precondition itself (see A-16). Not modeled: monitor-trap-flag injection (type 7), VM-entry MSR loading before injection, IRET-triggered clearing of blocking-by-NMI, and delivery into a legacy (32-bit protected-mode) guest, which hits baseline gem5's pre-existing "legacy mode interrupts not implemented in microcode" panic. No automated test exercises this path; it requires the full-system resources. |
...
| A-19 | Missing feature / classification error | SDM 26.2.1.3, 26.4, 26.5.1; gem5 microcode fault-delivery ROM | VM-entry event injection was unconditionally rejected (the entry-interruption-information field was required to be zero), the guest-interruptibility-state field was required to be entirely zero rather than validated bit by bit, and a nonzero VM-entry MSR-load count was misclassified as a pre-entry invalid-control-field VMfail instead of the late MSR-loading failure class that SDM 26.4 requires. | Validate the injection descriptor's type/vector/error-code/instruction-length fields and its interaction with MOV-SS blocking (error 26); factor the existing native fault-delivery microcode dispatch (`X86FaultBase::invoke`) into a shared `deliverInterruptOrException` helper and reuse it to push the requested event onto the guest immediately after guest-state loading, before the caller resumes fetch at guest RIP; classify a nonzero VM-entry MSR-load count as a late failure with exit reason 34; validate the interruptibility-state field's individual bits (reserved bits, STI/MOV-SS exclusivity, STI-requires-IF) instead of forcing it to zero; and track blocking-by-NMI across VM entry/exit as a direct effect of NMI injection. Full clean rebuild (0 warnings) and the full VMX unit-test suite were re-run with no regressions; no automated test exercises the new injection path itself, which requires the full-system resources. | **Implemented; integration verification pending** |
| A-20 | Missing validation / incomplete feature | SDM 26.2.1.3 (deliver-error-code and instruction-length checks), 24.4.2 and 6.8.3 (interruptibility-state field, `MOV SS` shadow) | The deliver-error-code bit was only checked one way (it forced hardware-exception type, but a hardware exception with a vector that architecturally pushes an error code, e.g. `#GP`, could be injected with the bit clear, and any vector could be injected with it set); a VM-entry instruction length of 0 for a software-class injection was always accepted instead of being gated on `IA32_VMX_MISC[30]`; and the STI-blocking/MOV-SS-blocking interruptibility bits were accepted as valid input but never actually withheld interrupt delivery, so a guest hypervisor's request for a one-instruction shadow after VM entry was silently dropped. | Add a vector table (`hardwareExceptionRequiresErrorCode`: `#DF`/`#TS`/`#NP`/`#SS`/`#GP`/`#PF`/`#AC`) and require the deliver-error-code bit to equal `type == HardwareException && vector-requires-it` exactly; read `IA32_VMX_MISC[30]` and reject a zero instruction length for software-class injections when it is clear (always, in this model); and honor STI-blocking/MOV-SS-blocking for one guest instruction after entry by tying the shadow to the entry RIP (`VmxState::refreshInterruptShadow`, consulted from `X86ISA::Interrupts::checkInterrupts`) rather than a general instruction-boundary hook, since gem5 has none — MOV-SS-blocking additionally withholds NMI per SDM 6.8.3. This is scoped to VMX non-root execution; native `STI`/`MOV SS` shadowing and the VMLAUNCH/VMRESUME precondition itself (A-16) remain unmodeled. The error-code/vector table and the instruction-length/`IA32_VMX_MISC[30]` gate were factored into `vmx_utils.hh` and now have dedicated gtest coverage (`VmxEventInjection.*` in `vmx_utils.test.cc`); the interrupt-shadow and injection-delivery behavior itself still has no automated coverage, since exercising it needs a full VM entry/exit under `tests/gem5/vmx` (see that suite's README for the current full-system bring-up status). Full clean rebuild (0 warnings) and the full VMX unit-test suite were re-run with no regressions. | **Implemented; integration verification pending** |
...
- VM-entry/exit MSR load/store list execution (a nonzero VM-entry MSR-load
  count now fails with the correct late exit reason, but no entries are ever
  loaded); monitor-trap-flag event injection; and delivery of an injected
  event into a legacy (32-bit protected-mode) guest;
...
- a general, native `STI`/`MOV SS` interrupt-shadow model (the guest-facing
  shadow loaded from the interruptibility-state field at VM entry is
  implemented, scoped to VMX non-root execution only; see "VM-entry event
  injection" above), the VM-entry precondition while blocked by `MOV SS`
  (A-16), async interrupt/NMI tests, and triple-fault/shutdown behavior;
