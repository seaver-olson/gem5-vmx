...
| INVD / GETSEC / XSETBV | Partial interception only | Higher-priority recognition and privilege faults precede each distinct non-root VM exit: INVD checks CPL; XSETBV checks CR4.OSXSAVE then CPL; and GETSEC checks CR4.SMXE. Root execution after those checks retains baseline gem5's warning/no-op implementation and is outside this VMX research contract. |
